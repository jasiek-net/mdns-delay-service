...
// change ansi string to network query
...
unsigned char *get_record(unsigned char *query, int query_type, ssize_t *len) {
    printf("Resolving %s" , query);

    unsigned char buf[MAX_BUF_SIZE], *qname;
    ...
    strncpy((char *) result, (char *) buf, size);
...
void *m_dns(void *arg) {
  ...
  ssize_t snd_len, len;
  unsigned char *msg = get_record((unsigned char *) "_opoznienia._udp.local.", T_PTR, &len);
  parse_msg(msg);
  printf("size: %d\n", len);
  // while(1) {
    snd_len = sendto(sock, msg, len, flags, &mdns_addr, addr_len);
